...
public class InfiniteScroll : MonoBehaviour, IBeginDragHandler, IDragHandler, IScrollHandler
{
    ...
    {
        lastDragPosition = eventData.position;     ///son konum = kaydırdığım dokunduğum ilk an konumu konum
    }
    ...
    private void HandleVerticalScroll()
    {
        ...
        if (positiveDrag)
        {
            ...
            newPos.y = (endItem.position.y - scrollContent.ChildHeight * (7.3f) + scrollContent.ItemSpacing);
        }
        else
        {
    ...
    private bool ReachedThreshold(Transform item)
    {
        ...
        if (scrollContent.Vertical)
        {
            float posYThreshold = transform.position.y + scrollContent.Height * 7.3f + outOfBoundsThreshold;
            float negYThreshold = transform.position.y - scrollContent.Height * 7.3f - outOfBoundsThreshold;
            return positiveDrag ? item.position.y - scrollContent.ChildWidth * 7.3f > posYThreshold :
                item.position.y + scrollContent.ChildWidth  < negYThreshold;
        }
        else
        {
