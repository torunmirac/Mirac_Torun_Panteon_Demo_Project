...
public class UIManager : InstanceManager<UIManager>
{
    ...
    [SerializeField] float OnProductionMenu, OffProductionMenu, OnItemOptions, OffItemOptions, OnInformationMenu, OffInformationMenu, OnProductSoldierMenu, OffProductSoldierMenu;
    ...
    {
        //IsInstantiateBuild(PowerPlant, manager.powerplantList);
    ...
    {
        //InstantiateSoldier(blueSoldier, soldierManager.blueSoldier);
    ...
    {
        ProductionMenu.transform.DOMoveX(OnProductionMenu, 0.5f).SetEase(Ease.InCubic).OnComplete(() =>
        {
    ...
    {
        ProductionMenu.transform.DOMoveX(OffProductionMenu, 0.5f).SetEase(Ease.InCubic).OnComplete(() =>
        {
    ...
    {
        LeanTween.moveX(InformationMenu, OnInformationMenu, .5f).setEaseInCubic();

    }

    void IsOffInformationMenu()
    {
        LeanTween.moveX(InformationMenu, OffInformationMenu, .5f).setEaseInCubic();
    ...
    void IsOnItemOptions()
    {
        ...
        ItemOptions.transform.DOMoveY(OnItemOptions, 0.5f).SetEase(Ease.InCubic);
        ProductionMenu.transform.GetChild(0).GetComponent<Button>().interactable = false;
    }

    void IsOffItemOptions()
    {
        ItemOptions.transform.DOMoveY(OffItemOptions, 0.5f).SetEase(Ease.InCubic).OnComplete(() =>
        {
    ...
    {
        ProductSoldierMenu.transform.DOLocalMoveY(OnProductSoldierMenu, 0.5f).SetEase(Ease.InCubic);
    }

    void IsOffProductSoldierMenu()
    {
        ProductSoldierMenu.transform.DOLocalMoveY(OffProductSoldierMenu, 0.5f).SetEase(Ease.InCubic);
    }
    #endregion
