...
public class PowerPlantControl : AbstractNumber

{
    ...
    private void Awake()
    {
        ...
        objectPools3 = FindObjectOfType<ObjectPools>();
    ...
    void OnInstantiateBuild()
    {
        ...
        {
            for (int i = 0; i < manager3.powerplantList.Count; i++)
            {
                if (!manager3.selectedBuild.Contains(manager3.powerplantList[i]))
                {
                    manager3.powerplantList[i].GetComponent<PowerPlantControl>().Clickable = false;
    ...
    void OnDontMove()
    {
        ...
        foreach (var item in manager3.powerplantList)
        {
            item.GetComponent<PowerPlantControl>().Clickable = true;
    ...
    void OnResetGameObject()
    {
        ...
        if (objectPools3.passivePowerPlant.Contains(gameObject))
        {
    ...
    private void OnCollisionEnter2D(Collision2D other)
    {
        ...
        if (!soldierlist.Contains(other.gameObject) && currentMoveBuildEnum != MoveBuildEnum.Red)
        {
            ...
            {
                case "SoldierBlue":
                    CollisionSoldier(gameObject, ref health, 2, healthBar, healthText, manager3.powerplantList, manager3.constructedBuilding, objectPools3.passivePowerPlant, objectPools3.activePowerPlant);


                    break;
                case "SoldierGreen":
                    CollisionSoldier(gameObject, ref health, 5, healthBar, healthText, manager3.powerplantList, manager3.constructedBuilding, objectPools3.passivePowerPlant, objectPools3.activePowerPlant);


                    break;
                case "SoldierRed":
                    CollisionSoldier(gameObject, ref health, 10, healthBar, healthText, manager3.powerplantList, manager3.constructedBuilding, objectPools3.passivePowerPlant, objectPools3.activePowerPlant);
